...
if(!empty($room)) {
	...
	sql_free($res);
?>
<h3 ALIGN=CENTER><? echo $vocab[editroom] ?></h3>
<form action="edit_area_room.php" method="post">
<input type=hidden name="room" value="<?php echo $row["id"]?>">
<CENTER>
<TABLE>
<TR><TD><? echo $vocab[name] ?>:       </TD><TD><input type=text name="room_name" value="<?php
echo htmlspecialchars($row["room_name"]); ?>"></TD></TR>
<TR><TD><? echo $vocab[description] ?></TD><TD><input type=text name=description value="<?php
echo htmlspecialchars($row["description"]); ?>"></TD></TR>
<TR><TD><? echo $vocab[capacity] ?>:   </TD><TD><input type=text name=capacity value="<?php
echo $row["capacity"]; ?>"></TD></TR>
</TABLE>
<input type=submit name="change_room"
value="<? echo $vocab[change] ?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<input type=submit name="change_done" value="<? echo $vocab[backadmin] ?>">
</CENTER>
</form>
<?php } ?>
...
if(!empty($area))
{
	...
	sql_free($res);
?>
<h3 ALIGN=CENTER><? echo $vocab[editarea] ?></h3>
<form action="edit_area_room.php" method="post">
<input type=hidden name="area" value="<?php echo $row["id"]?>">
<CENTER>
<TABLE>
<TR><TD><? echo $vocab[name] ?>:       </TD><TD><input type=text name="area_name" value="<?php
