...
{
  const possibleNames = ['room', 'date', 'seconds'];
  for (const name of possibleNames)
  {
    if (jqObject.data(name) !== undefined)
    {
      return name;
    }
  }
...
const Table = {
  ...
  getRowNumber: function(y) {
      for (let i=0; i<Table.grid.y.data.length - 1; i++)
      {
  ...
  clearRowLabels: function() {
      ...
      {
        for (let i=0; i < Table.highlightRowLabels.rows.length; i++)
        {
  ...
  highlightRowLabels: function (el) {
    ...
    const elStartRow = Table.getRowNumber(el.offset().top);
    const elEndRow = Table.getRowNumber(el.offset().top + el.outerHeight());
    for (let i=0; i<Table.highlightRowLabels.rows.length ; i++)
    {
  ...
  ?>
  outside: function(p) {
    const headBottoms = $(Table.selector).find('thead').map(function () {
  ...
  overlapsBooked: function overlapsBooked(rectangle, stopAtFirst, ignoreRectangle)
  {
    ...
    for (let i=0; i<Table.bookedMap.length; i++)
    {
  ...
  scrollContainerBy: function(xCoord, yCoord) {
    const container = $(Table.selector).parent();
    <?php
  ...
  size: function() {
    ...
    ?>
    const table = $(Table.selector);
    const td = table.find('tbody tr:first-child td:first-of-type');
    ...
    ?>
    const rtl = ((table.css('direction') !== undefined) && (table.css('direction').toLowerCase() === 'rtl'));
    const resolution = table.data('resolution');
    Table.grid.x = {};
    Table.grid.x.data = [];
    <?php
    // We need :visible because there might be hidden days
    ?>
    const columns = table.find('thead tr:first-child th:visible').not('.first_last');
    ...
    Table.grid.y = {};
    Table.grid.y.data = [];
    const rows = table.find('tbody th:first-child');
    rows.each(function() {
...
$(document).on('page_ready', function() {
  ...
  $(Table.selector).on('tableload', function() {
    ...
    const downHandler = function(e) {
      ...
      if (!args.isBookAdmin)
      {
            ...
            ((args.view === 'day') && <?php echo ($auth['only_admin_can_select_multiroom']) ? 'true' : 'false'?>))
        {
          ...
          {
            ?>
            const slotHeight = jqTarget.outerHeight();
          ...
          {
            ?>
            const slotWidth = jqTarget.outerWidth();
    ...
    const upHandler = function(e) {
      ...
      mouseDown = false;
      e.preventDefault();
      const tolerance = 2; <?php // px ?>
      const box = downHandler.box;
      const params = Table.getBookingParams(box);
      ...
      ?>
      let queryString = 'drag=1';  <?php // Says that we've come from a drag select ?>
      queryString += '&area=' + args.area;
      queryString += '&start_seconds=' + params.seconds[0];
      queryString += '&end_seconds=' + params.seconds[params.seconds.length - 1];
      if (args.view === 'day')
      {
        for (let i=0; i<params.room.length; i++)
        {
    ...
    ?>
    const resize = function(event, ui)
    {
      let closest;
      const rectangle = {};
      ...
      const overlappedElements = Table.overlapsBooked(rectangle, false, originalRectangle);
    ...
    ?>
    const resizeStart = function (event, ui) {
      resizeStart.originalOffset = ui.element.offset();
      resizeStart.oldParams = Table.getBookingParams(ui.originalElement.find('a'));
      <?php
      // Add a class so that we can disable the highlighting when we are resizing (the flickering is a bit annoying)
      ?>
      table.addClass('resizing');
    };
    ...
    ?>
    const resizeStop = function(event, ui)
    {
      <?php
      ...
      const booking = ui.element.find('a');
      <?php // get the booking id and type ?>
      data.id = booking.data('id');
      data.type = booking.data('type');
      <?php // get the other parameters ?>
      const oldParams = resizeStart.oldParams;
      const newParams = Table.getBookingParams(booking);
      ...
      else  <?php // it's 'week' ?>
      {
        ...
        const onlyAdminCanBookRepeat = <?php echo ($auth['only_admin_can_book_repeat']) ? 'true' : 'false';?>;
             ...
             function(result) {
                ...
                if (!result.valid_booking)
                {
                  ...
                  if (result.violations.errors.length > 0)
                  {
                    ...
                    const rulesList = getErrorList(result.violations.errors);
